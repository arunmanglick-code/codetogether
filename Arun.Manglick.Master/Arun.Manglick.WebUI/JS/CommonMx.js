...
document.onkeydown = function(myEvent) {
    if (navigator.appName == "Microsoft Internet Explorer") {
        ...
        if (window.event && window.event.keyCode == 505) {
            ...
            if (window.event && window.event.keyCode == 8 && isBackspaceActive) { // try to cancel the backspace
                window.event.cancelBubble = true;
                window.event.returnValue = false;
                return false;
            }
            ...
            // Skip check for ALT + <-
            if (window.event && window.event.keyCode == 37 && isBackArrowActive) { // try to cancel the backspace 
                window.event.cancelBubble = true;
                window.event.returnValue = false;
                return false;
            }
        ...
        // Skip check for BACKSAPCE 
        if (myEvent.which == 8 && isBackspaceActive) { // try to cancel the backspace
            myEvent.cancelBubble = true;
            myEvent.returnValue = false;
            return false;
        }
        ...
        // Skip check for ALT + <-
        if (myEvent.which == 37 && isBackArrowActive) { // try to cancel the backspace 
            myEvent.cancelBubble = true;
            myEvent.returnValue = false;
            return false;
        }
