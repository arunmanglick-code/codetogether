...
function validateDevices(ddlDevices,txtDeviceAddress,ddlCarrier,txtGateway, txtInitialPause)
{    
    var setfocus = "" ;
    var device = trim(document.getElementById(ddlDevices).value);
...
/* For the device type Skytel / Pager USA it should allow space at 10 th index*/
    function isNumericKeyOrSpace(txtDeviceAddress)
    {
        if(document.getElementById(txtDeviceAddress).value.length == 10)
        {
            ...
            return isNumericKeyStrokes(hdnTextChangedClientID);                
    } 
...
//Sets Postback Variable value to True
    ...
    {       
        var flag = document.getElementById(hdnTextChangedClientID).value
        if (flag == "true")
        {
