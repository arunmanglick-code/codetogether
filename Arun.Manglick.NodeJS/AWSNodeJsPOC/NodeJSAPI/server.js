...
// GetAllTags - http://localhost:8080/lookUpTags
...
const sp_GetAllTags= 'sp_get_AllTags';
...
// -----------------------------------------------------------------
app.get('/lookUpTags',(req,res)=>{
  var tagName = req.params.TagName;

  var query = 'CALL ' + database + '.' + sp_GetAllTags + '();'
  var rowsResult = null;
  console.log(query);
    ...
    connection.query(query, function(err, rows)
      {
        if (err)
        {
          ...
          // connection.end();
          var rowsResult = JSON.parse(JSON.stringify(rows));
          if(rowsResult.length > 0)
          {
            console.log('TAGs Found successful');
            res.send(rowsResult[0]);
...
//----------------------------------------------------------------
app.listen(4090, () => {
  console.log(`Started up at port 4090`);
});
